...
def read_adf11(fil):
    ...
    while adf11['input_file']['dens_grid'].size < adf11['input_file']['num_dens']:
        adf11['input_file']['dens_grid'] = np.append(adf11['input_file']['dens_grid'], np.array(
            list(map(float, re.findall('(.\d*\.\d+)', f.readline())))))
    ...
    while adf11['input_file']['temp_grid'].size < adf11['input_file']['num_temp']:
        adf11['input_file']['temp_grid'] = np.append(adf11['input_file']['temp_grid'], np.array(
            list(map(float, re.findall('(.\d*\.\d+)', f.readline())))))
    ...
    while len(gcr_line.strip()) != 0 and 'C-' not in gcr_line:
        ...
        if ('---' in gcr_line):
            ...
            if ('scd' in fil or 'acd' in fil or 'ccd' in fil):
                if (len(stage_id) > 1):
                    tmp = stage_id[0]
