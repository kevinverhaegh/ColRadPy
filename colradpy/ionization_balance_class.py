    ...
    def __init__(self, fils, metas, temp_grid, dens_grid, htemp_grid=np.array([]), hdens_grid=np.array([]),
        ...
        if (adf11_files):  # adf11 specific formatting
            ...
            for i in range(0, len(fils)):
                ...
                if (type(use_cx) == bool):
                    self.data['user']['use_cx'] = np.ones(len(self.data['cr_data']['gcrs']), dtype=bool)
    ...
    def populate_ion_matrix(self):
        ...
        if (self.data['user']['temp_dens_pair']):
            ...
            for i in range(0, len(self.data['cr_data']['gcrs'])):
                ...
                self.data['ion_matrix'][m + diag_met[0], m + diag_met[1]] = self.data['ion_matrix'][
                                                                                m + diag_met[0], m + diag_met[1]] \
                                                                            - np.sum(
                    self.data['cr_data']['gcrs'][str(i)]['qcd'], axis=1)

                self.data['ion_matrix'][m:m + num_met, m:m + num_met, :] = self.data['ion_matrix'][m:m + num_met,
                                                                           m:m + num_met, :] \
                                                                           + self.data['cr_data']['gcrs'][str(i)][
                                                                               'qcd'].transpose(1, 0, 2)
                ...
                self.data['ion_matrix'][m + diag_met[0], m + diag_met[1]] = self.data['ion_matrix'][
                                                                                m + diag_met[0], m + diag_met[1]] \
                                                                            - np.sum(
                    self.data['cr_data']['gcrs'][str(i)]['scd'], axis=1)
